...
public record struct Vector2D(double X, double Y)
{
    ...
    public readonly Vector2D Normalized => Magnitude == 0 ? new(0, 0) : this * (1.0 / Magnitude);
...
public record AngleDefinition(Angle Angle, string DisplayValue, double Radius, double TextOffsetScale = 1.5, string Color = "var(--warning-accent)", string FontWeight = "normal");
